...
def test_create():
    ...
    #запрашиваем по id
    responses = requests.get(f"{base_url}/projects/{id}", headers=main_heders)
...
def test_update():
    ...
    responses = requests.put(f"{base_url}/projects/{id}", headers=main_heders, json=body_new)
    assert responses.status_code == 200
...
def test_receipt_id_negative():
    ...
    id = responses_body["id"]
    # запрашиваем по id
    responses = requests.get(f"{base_url}/projects/99999999", headers=main_heders)
    assert responses.status_code == 400
    responses_body = responses.json()
    title = responses_body["title"]
    assert title == body["title"]
